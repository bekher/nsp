...
internals.optionSchema = Joi.object({
  ...
  proxy: Joi.string()
}).or(['package', 'shrinkwrap', 'packageUrl']);
...
module.exports = function (options, callback) {
  ...
  var doCheck = function(options) {
    if (typeof options.packageUrl === 'string' && !! options.packageUrl) {
      delete options.shrinkwrap;
    }
    else if (typeof options.shrinkwrap === 'string') {
      try {
  ...
  if (typeof options.packageUrl === 'string' && !! options.packageUrl) {
    try {
      Wreck.get(options.packageUrl, function(err, res, payload) {
        if (err) {
          return callback(err);
        }
        options.package = payload.toString();
        options.package = SanitizePackage(JSON.parse(options.package));
        console.log("using github option");
        doCheck(options);
      });
    }
    catch (e) {
    ...
    if (typeof options.package === 'string') {
      try {
        ...
        console.log("using require option");
      }
      catch (e) {
